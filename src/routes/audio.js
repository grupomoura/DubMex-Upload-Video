...
audio.get('/download/audio', authenticate, async (req, res) => {
  ...
  sanitizedAudioListName = channelName.replace(/[^\w\s]/gi, '');
  ...
  const channelPath = path.join(basePath, endpointFolderName, sanitizedAudioListName);
  ...
  if (fs.existsSync(filePath)) {
    return res.status(200).json({
      ...
      downloadUrl: `${req.protocol}://${req.get('host')}/download/audio/${endpointFolderName}/${sanitizedAudioListName}/${fileName}`
    });
  }
  ...
  if (!(await doesChannelFolderExistInDrive(channelFolderId, sanitizedAudioListName))) {
    channelSubFolderId = await createFolder(sanitizedAudioListName, channelFolderId);
  } else {
    const drive = google.drive({ version: 'v3', auth: jwtClient });
    const response = await drive.files.list({
      q: `name = '${sanitizedAudioListName}' and mimeType = 'application/vnd.google-apps.folder' and trashed = false and '${channelFolderId}' in parents`,
      fields: 'files(id)'
    });
  ...
  if (await doesFileExistInDrive(channelSubFolderId, fileName)) {
    ...
    return res.status(200).json({
      message: 'O áudio já foi baixado.',
      downloadUrl: await getFolderLink(sanitizedAudioListName)
    });
  }

  const fileId = await uploadFileToDrive(filePath, fileName, channelSubFolderId);
  const folderLink = await getFolderLink(sanitizedAudioListName);
